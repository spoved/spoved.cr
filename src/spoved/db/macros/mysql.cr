abstract class Spoved::DB::Model::MySQL < Spoved::DB::Model
  macro database_model(table, primary_id, attributes, use_expire = false)
    ...
    alias ValTypes = String {% for name, val in attributes %} | {{val}} {% end %}
    ...
    protected def self.non_id_attributes
      {{ @type }}.attributes.reject &.== {{ @type }}.primary_key_name
    end
    ...
    private def _update_record
      ...
      logger.debug { sql }
      db.exec(sql,
        {% for key in attributes.keys.reject { |x| x.id == primary_id.id } %}
        _{{key}}_for_mysql ,
        {% end %}
    ...
    def get(%name : Symbol | String) : ValTypes
      case %name.to_s
      {% for name, val in attributes %}{% if name.id != primary_id.id %}
      when {{name.stringify}}
        self.{{name}}
      {% end %}{% end %}
      else
